...
The current implementation allows for independent models for the estimated transition intensities, including Poisson regression using \code{stats::glm} and \code{mgcv::gam}, parametric survival models (\code{flexsurv::flexsurvreg}), flexible parametric survival models (\code{rstpm2::stpm2} and \code{flexsurv::flexsurvspline}), and penalised survival models (\code{rstpm2::pstpm2} and \code{survPen::survPen}, where the latter is on the log-hazard scale). The implementation also allows for  for functions to represent the transitions and for a transition to be set to zero. The function scales reasonably well with multiple covariates within the same system of ODEs. A large number of covariate patterns will be blocked and run on separate cores.
...
\section*{Appendix B: Regression model specifications}

\begin{table}[!ht]
  \centering
\begin{minipage}{\textwidth}
  \begin{tabular}{llll}
    Class & Model & R function            & Specification\footnote{Notes: Formally, Poisson regression models for rates rather than hazards. $\eta(t,\bm{x})$ and $\eta(\bm{x})$ are linear predictors for time $t$ and covariates $\bm{x}$; the generality of the linear predictor may be constrained by the model class (e.g. \texttt{flexsurv::flexsurvspline} only allows for spline interactions between time and covariates). $S_0(t)$ is a baseline survival model; for \texttt{flexsurv::flexsurvreg} it is from a parametric family, while for \texttt{rstpm2::aft} the log cumulative hazards is based on splines. The generalised survival models also allow for other transformations, including proportional odds, probit and additive hazards models.} \\ \hline
  ...
  \end{tabular}
\end{minipage}
\caption{Regression model specification}
  \label{tab:models}
\end{table}
