...
\Abstract{
  ...
  probabilities, length of stay and their associated standard
  errors. These methods have applications to a range of disciplines,
  including epidemiology, causal inference and economics.
...
\begin{align}
\frac{d\boldsymbol{L}(t_0,t)}{dt} &= \boldsymbol{P}(t_0,t)\boldsymbol{u}(t)e^{-\lambda t} \label{eq:los}
\end{align}
In health economics, the sum across the states gives the discounted, quality-adjusted life-years $\text{QALY}(t_0,t)=\boldsymbol{L}(t_0,t)\boldsymbol{1}$. If $u_j(v)=1$ and $\lambda=0$ then $L_{0j}(t_0,t)$ is the sojourn time for state $j$ (also called the length of stay; see \cite{Jackson_2016}) . We can also augment the system to model for costs per unit time for being in state $i$ at time $t$, represented by the vector function $\boldsymbol{c}(t)=(c_{i}(t))$ and model for costs for transitions from state $i$ to state $j$ at time $t$, represented by the matrix $\boldsymbol{\mathcal{C}}(t)=(\mathcal{C}_{i,j}(t))$. Then the cumulative costs $\boldsymbol{C}(t_0,t)$ can be represented by the equations
\begin{align}
\frac{d\boldsymbol{C}(t_0,t)}{dt} &= \left(\boldsymbol{P}(t_0,t)\boldsymbol{c}(t)+
                                    \boldsymbol{P}(t_0,t)(\boldsymbol{Q}(t)\circ \boldsymbol{\mathcal{C}}(t))\right)e^{-\lambda t} \label{eq:costs}
\end{align}
where $\boldsymbol{Q}(t)\circ \boldsymbol{\mathcal{C}}(t)$ is the Hadamard (element-wise) product between matrix $\boldsymbol{Q}(t)$ and matrix $\boldsymbol{\mathcal{C}}(t)$. The total costs are summed across the health states, such that $\text{Costs}(t_0,t)=\boldsymbol{C}(t_0,t)\boldsymbol{1}$.
...
\begin{align}
  \frac{d\boldsymbol{L}'_m(t_0,t)}{dt} &= \boldsymbol{P}'_m(t_0,t)\boldsymbol{u}(t)e^{-\lambda t} \label{eq:dLudt}
\end{align}
and the gradients for the cumulative costs, such that
\begin{align}
  \frac{d\boldsymbol{C}'_m(t_0,t)}{dt} &= \left(\boldsymbol{P}'_m(t_0,t)\boldsymbol{c}(t)+
                                         \left((\boldsymbol{P}'_m(t_0,t)\boldsymbol{Q}(t) + \boldsymbol{P}(t_0,t)\boldsymbol{Q}'_m(t))\circ \boldsymbol{\mathcal{C}}(t)\right)\right)e^{-\lambda t} \label{eq:dCudt}
\end{align}

The initial values for Equations~(\ref{eq:dPudt})--(\ref{eq:dCudt})
are
$\boldsymbol{P}'_m(t_0,t_0)=\boldsymbol{L}'_m(t_0,t_0)=\boldsymbol{C}'_m(t_0,t_0)=\boldsymbol{0}$.
Note that Equations~(\ref{eq:dPudt})--(\ref{eq:dCudt}) only account
for uncertainty in the transition intensities. These equations can be
extended to include the uncertainty in the utility functions
$\boldsymbol{u}(t)$ and the cost functions $\boldsymbol{c}(t)$ and
...
\begin{algorithm}[H]
  ...
  \Input{$\boldsymbol{Q}(t), \boldsymbol{Q}'_m(t)\,\forall m, \boldsymbol{u}(t), \lambda, \boldsymbol{c}(t), \boldsymbol{\mathcal{C}}(t), t_0, t_\text{max}$} \Output{$\boldsymbol{P}(t_0,t),\boldsymbol{L}(t_0,t),\boldsymbol{C}(t_0,t),\boldsymbol{P}'_m(t_0,t),\boldsymbol{L}'_m(t_0,t),\boldsymbol{C}'_m(t_0,t)\,\forall m\ \text{for}\ t\in\left[t_0,t_\text{max}\right)$}
  \Begin{
    $\boldsymbol{P}(t_0,t_0)=\boldsymbol{I}$\; $\boldsymbol{L}(t_0,t_0)=\boldsymbol{C}(t_0,t_0)=\boldsymbol{0}$\;
  $\boldsymbol{P}'_m(t_0,t_0)=\boldsymbol{L}'_m(t_0,t_0)=\boldsymbol{C}'_m(t_0,t_0)=\boldsymbol{0}$\;
...
\end{align*}
where the division $\left(\frac{\boldsymbol{\phi}'_m}{\boldsymbol{\phi}}\right)_j=\phi'_{m,j}/\phi_j$m which is element-wise division. We can also consider incremental cost-effectiveness ratios, estimated by 
\begin{align*}
