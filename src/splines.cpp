...
namespace rstpm2 {
  ...
  SplineBasis::SplineBasis(int order) : order(order) {
    ...
    SplineBasis::SplineBasis(vec knots, int order) : order(order), knots(knots) {
      ordm1 = order - 1;
      nknots = knots.size();
      ncoef = nknots - order;
      rdel = vec(ordm1);
    ...
    vec SplineBasis::basis_funcs(double x)
    {
      vec b(order);
      diff_table(x, ordm1);
      b(0) = double(1);
    ...
    vec SplineBasis::eval(double x, int ders) {
      vec val(ncoef);
      val = zeros(ncoef);
      set_cursor(x);
      int io = curs - order;
    ...
    mat SplineBasis::basis(vec x, int ders) {
  ...
  bs::bs(vec boundary_knots, vec interior_knots, int intercept) :
  ...
  vec bs::eval(double x, int ders) {
	...
	vec = SplineBasis::eval(x, ders).subvec(1-intercept,df-intercept);
      }
      return vec;
    }
    mat bs::basis(vec x, int ders) {
      mat mat(x.size(), df);
      for (size_t i=0; i<x.size(); i++) {
  ...
  ns::ns() {} // default constructor
    ...
    // }
    ns::ns(vec boundary_knots, vec interior_knots, mat _q_matrix, int intercept, int cure) :
    ...
    mat ns::basis(vec x, int ders) {
      mat mat(x.size(), this->df-2-cure);
      for (size_t i=0; i<x.size(); i++) {
