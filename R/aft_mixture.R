...
setClass("aft2", representation(args="list"), contains="mle2")
...
aft_mixture <- function(formula, data, smooth.formula = NULL, df = 3,
    ...
    negll <- function(beta) {
        ...
        localargs$init <- beta
        ## return(aft_model_output2(localargs)[[1]])
        return(.Call("aft_model_output2", localargs, PACKAGE="rstpm2"))
    }
    gradient <- function(beta) {
        localargs <- args
        localargs$return_type <- "gradient"
        localargs$init <- beta
        ## return(aft_model_output2(localargs))
        return(as.vector(.Call("aft_model_output2", localargs, PACKAGE="rstpm2")))
    }
    ...
    if (delayed && use.gr) { # initial search using nmmin
        ...
        args$maxit <- 50
        ## fit <- aft_model_output2(localargs)
        fit <- .Call("aft_model_output2", args, PACKAGE="rstpm2")
...
predict.aft2 =  function(object,newdata=NULL,
              ...
              if (type == "gradh") {
                  return(predict.aft2.ext(object, type="gradh", time=time, X=X, XD=XD))
              }
...
setMethod("predict", "aft2", predict.aft2)
cloglog <- function(x) log(-log(x))
cexpexp <- function(x) exp(-exp(x))

## setMethod("predictnl", "aft2",
...
#     # transformation
#     
#     g <- \(x) f(x*scale)*rep(exp(x)*scale, each = length(object@args$event))
#     # browser()
#     g(roots) %*% weights
#   }
#     
...
#     
#     model_density <-  predict.aft2(object,
#                                    type = "density",
#                                    newdata = newdata)
#     
#  
...
plot.aft2.meansurv <- function(x, y=NULL, times=NULL, newdata=NULL, type="meansurv", exposed=NULL, add=FALSE, ci=!add, rug=!add, recent=FALSE,
...
plot.aft2.base <- 
          function(x,y,newdata=NULL,type="surv",
              ...
              if (type %in% c("meansurv","meansurvdiff","af")) {
                  return(plot.aft2.meansurv(x,times=times,newdata=newdata,type=type,xlab=xlab,ylab=ylab,line.col=line.col,ci.col=ci.col,
...
setMethod("plot", signature(x="aft2", y="missing"),
          function(x,y,newdata=NULL,type="surv",
                   xlab=NULL,ylab=NULL,line.col=1,ci.col="grey",lty=par("lty"),
                   add=FALSE,ci=!add,rug=!add,
                   var=NULL,exposed=incrVar(var),times=NULL,...)
              plot.aft2.base(x=x, y=y, newdata=newdata, type=type, xlab=xlab,
                              ylab=ylab, line.col=line.col, lty=lty, add=add,
                              ci=ci, rug=rug, var=var, exposed=exposed, times=times, ...)
          )
predict.aft2.ext <- function(obj, type=c("survival","haz","gradh"),
    ...
    localargs$XD <- XD
    localargs$time <- time
    as.matrix(.Call("aft_model_output2", localargs, PACKAGE="rstpm2"))
    ## as.matrix(aft_model_output2( localargs))
}
